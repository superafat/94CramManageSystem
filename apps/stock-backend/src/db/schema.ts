import { pgTable, varchar, text, timestamp, boolean, integer, decimal, uuid, unique } from 'drizzle-orm/pg-core';
...
export const stockInventory = pgTable('stock_inventory', {
  ...
  unq: unique().on(table.warehouseId, table.itemId),
}));
...
// Stock Users
export const stockUsers = pgTable('stock_users', {
  id: uuid('id').primaryKey().defaultRandom(),
  tenantId: uuid('tenant_id').notNull(),
...
// Stock Tenants
export const stockTenants = pgTable('stock_tenants', {
  id: uuid('id').primaryKey().defaultRandom(),
  name: varchar('name', { length: 200 }).notNull(),
