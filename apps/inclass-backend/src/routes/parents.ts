...
const ok = <T>(data: T, status?: 200 | 201) =>
...
parentsRouter.post('/', zValidator('json', parentSchema), async (c) => {
  try {
    ...
    if (!student || student.schoolId !== schoolId) {
      return c.json(fail('Student not found'), 404)
    }
    ...
    if (existingParent) {
      return c.json(fail('Parent with this phone number already exists for this student'), 400)
    }
...
parentsRouter.post('/line-bind', zValidator('json', lineBindSchema), async (c) => {
  try {
    ...
    if (!parent) {
      return c.json(fail('Parent not found with this phone number'), 404)
    }
    ...
    if (body.lineUserId !== parent.lineUserId) {
      ...
      if (existingLine && existingLine.id !== parent.id) {
        return c.json(fail('LINE account already bound to another parent'), 400)
      }
    }
...
parentsRouter.get('/:id', zValidator('param', parentIdParamSchema), async (c) => {
  try {
    ...
    if (!parent) return c.json(fail('Parent not found'), 404)

    const [student] = await db.select().from(students).where(eq(students.id, parent.studentId))
    if (!student || student.schoolId !== schoolId) return c.json(fail('Parent not found'), 404)

    return c.json(ok({ parent }))
  } catch (e) {
    console.error('[API Error]', c.req.path, 'Error fetching parent:', e instanceof Error ? e.message : 'Unknown error')
    return c.json(fail('Failed to fetch parent'), 500)
  }
})
...
parentsRouter.put(
  ...
  try {
    ...
    if (body.studentId && body.studentId !== parent.studentId) {
      ...
      if (!newStudent || newStudent.schoolId !== schoolId) return c.json(fail('Invalid student ID'), 400)
    }
    ...
    if (body.lineUserId !== undefined) updateData.lineUserId = body.lineUserId
    if (body.relationship !== undefined) updateData.relation = body.relationship

    const [updated] = await db.update(parents).set(updateData).where(eq(parents.id, id)).returning()
    return c.json(ok({ parent: updated }))
  } catch (e) {
    console.error('[API Error]', c.req.path, 'Error updating parent:', e instanceof Error ? e.message : 'Unknown error')
    return c.json(fail('Failed to update parent'), 500)
  }
})
...
parentsRouter.delete('/:id', zValidator('param', parentIdParamSchema), async (c) => {
  try {
    ...
    return c.json(fail('Failed to delete parent'), 500)
  }
})
