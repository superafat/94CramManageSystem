...
export default function MakeupClassesPage() {
  ...
  const loadRecords = useCallback(async () => {
    ...
    setError(null)
    try {
      const data = await api.fetch(`/api/makeup-classes?status=${activeTab}`)
      const payload = (data && typeof data === 'object' && 'data' in data)
        ? (data as { data: { makeupClasses: MakeupClass[] } }).data
        : data
      setRecords((payload as { makeupClasses: MakeupClass[] }).makeupClasses || [])
    } catch (err) {
  ...
  const loadTeachers = useCallback(async () => {
    try {
      const data = await api.fetch('/api/teachers')
      const payload = (data && typeof data === 'object' && 'data' in data)
        ? (data as { data: { teachers: Teacher[] } }).data
        : data
      setTeachers((payload as { teachers: Teacher[] }).teachers || [])
    } catch {
      // 忽略錯誤，下拉選單可為空
    }
  }, [])
  ...
  const loadStudents = useCallback(async () => {
    try {
      const data = await api.fetch('/api/students')
      const payload = (data && typeof data === 'object' && 'data' in data)
  ...
  useEffect(() => {
    const loadStats = async () => {
      try {
        const data = await api.fetch('/api/makeup-classes/stats')
        const payload = (data && typeof data === 'object' && 'data' in data)
          ? (data as { data: { stats: { pending: number; scheduled: number; completedThisMonth: number } } }).data
          : data
        if ((payload as { stats?: { pending: number; scheduled: number; completedThisMonth: number } }).stats) {
          setStats((payload as { stats: { pending: number; scheduled: number; completedThisMonth: number } }).stats)
        }
      } catch {
  ...
  const searchSlots = async () => {
    ...
    try {
      ...
      const data = await api.fetch(`/api/makeup-classes/slots?${params.toString()}`)
      const payload = (data && typeof data === 'object' && 'data' in data)
        ? (data as { data: { slots: MakeupSlot[] } }).data
        : data
      setAvailableSlots((payload as { slots: MakeupSlot[] }).slots || [])
    } catch {
      setAvailableSlots([])
    } finally {
  ...
  const assignToSlot = async (slotId: string, makeupClassId: string) => {
    ...
    try {
      await api.fetch(`/api/makeup-classes/${makeupClassId}`, {
        method: 'PUT',
  ...
  const submitCreateSlotAndAssign = async (makeupClassId: string) => {
    ...
    try {
      const slotData = await api.fetch('/api/makeup-classes/slots', {
        method: 'POST',
        body: JSON.stringify({
      ...
      const slotPayload = (slotData && typeof slotData === 'object' && 'data' in slotData)
        ...
        : slotData
      const newSlotId = (slotPayload as { slot?: MakeupSlot }).slot?.id
      if (newSlotId) {
        await api.fetch(`/api/makeup-classes/${makeupClassId}`, {
          method: 'PUT',
  ...
  const submitSchedule = async () => {
    ...
    setScheduleSubmitting(true)
    try {
      await api.fetch(`/api/makeup-classes/${scheduleTarget.id}/schedule`, {
        method: 'PUT',
        body: JSON.stringify({
  ...
  const batchAssignToSlot = async (slotId: string) => {
    setScheduleSubmitting(true)
    try {
      await api.fetch('/api/makeup-classes/batch-assign', {
        method: 'POST',
        body: JSON.stringify({
          slotId,
  ...
  const sendNotify = async (id: string) => {
    try {
      await api.fetch(`/api/makeup-classes/${id}/notify`, { method: 'POST' })
      showToast('通知已發送')
    } catch {
  ...
  const openNoticePdf = (id: string) => {
    window.open(`/api/makeup-classes/${id}/notice-pdf`, '_blank')
  }
  ...
  const submitAdd = async () => {
    ...
    try {
      await api.fetch('/api/makeup-classes', {
        method: 'POST',
        body: JSON.stringify({
  ...
  const markComplete = async (id: string) => {
    try {
      await api.fetch(`/api/makeup-classes/${id}/complete`, { method: 'PUT' })
      loadRecords()
    } catch {
      loadRecords()
    }
  ...
  const cancelMakeup = async (id: string) => {
    ...
    try {
      await api.fetch(`/api/makeup-classes/${id}/cancel`, { method: 'PUT' })
      loadRecords()
    } catch {
      loadRecords()
    }
  ...
  // Suppress unused variable warning for scheduleForm
  void scheduleForm
