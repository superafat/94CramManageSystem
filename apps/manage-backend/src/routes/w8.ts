...
import { sendScheduleChangeNotifications, sendGradeNotification, sendBillingReminder } from '../services/notification-scenarios'
...
w8Routes.get('/recommendations', requirePermission(Permission.SCHEDULE_READ),
...
w8Routes.post('/notify/grade-posted', requireRole(Role.ADMIN, Role.MANAGER),
...
w8Routes.post('/notify/billing-reminder', requireRole(Role.ADMIN, Role.MANAGER),
...
w8Routes.get('/notifications', zValidator('query', notificationsQuerySchema), async (c) => {
  try {
    const query = c.req.valid('query')
    const user = c.get('user')
    if (!user?.tenant_id) {
