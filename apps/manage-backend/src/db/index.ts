...
function setupConnectionMonitoring(client: ReturnType<typeof postgres>) {
  ...
  setInterval(() => {
    try {
      ...
      logger.error({ err: error }, '[DB Monitoring Error]')
    }
  }, 60000) // 每分钟
}
...
export async function executeWithMetrics<T>(
  ...
  try {
    ...
    if (duration > SLOW_QUERY_THRESHOLD) {
      metrics.slowQueries++
      logger.warn({ queryName, duration }, '[Slow Query]')
    }
...
function initializeDatabase() {
  ...
  try {
    ...
    checkDatabaseHealth().then(health => {
      if (health.healthy) {
        logger.info({ latency: health.latency }, '[DB] Health check passed')
      } else {
        logger.error({ error: health.error }, '[DB] Health check failed')
      }
    })
    ...
    metrics.errors++
    logger.error({ err: error }, '[DB] Failed to initialize database')
    throw error
  }
}
